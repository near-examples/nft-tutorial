...
export function toChangeNearBalance(account: NearAccount, amount: string, precision: string = "1 N"): Assertion {
  ...
  return {
    ...
    expected: () => `${description}${value.divRound(p).mul(p).toString()}`, // TODO: Replace toString with toHuman when fix to units-js arrives
