...
workspace.test("nft contract: nft metadata view", async (test, { nft_contract, root }) => {
  const expected = {
    base_uri: null,
    icon: null,
  ...
  test.deepEqual(await nft_contract.view("nft_metadata", { account_id: root }), expected);
});

workspace.test("nft contract: nft mint call", async (test, { nft_contract, alice }) => {
...
workspace.test("nft contract: nft approve call", async (test, { nft_contract, market_contract, alice }) => {
...
workspace.test("nft contract: nft approve call long msg string", async (test, { nft_contract, market_contract, alice }) => {
  await mintNFT(alice, nft_contract);
