...
import { defaultCallOptions, mintNFT, approveNFT, DEFAULT_GAS, payForStorage, placeNFTForSale, purchaseListedNFT, transferNFT } from "./utils";
...
test("cross contract: sell NFT listed on marketplace", async t => {
    ...
    t.deepEqual(result1.actual, result1.expected, "Should change user balances by sale_price of 0.3 N");
...
test("cross contract: reselling and royalties", async t => {
    ...
    t.deepEqual(purchase_result.actual, purchase_result.expected, "Should change user balances when doing NFT 1");
    ...
    t.deepEqual(purchase_result_2.actual, purchase_result_2.expected, "Should change balances by a total of 30 N");
});